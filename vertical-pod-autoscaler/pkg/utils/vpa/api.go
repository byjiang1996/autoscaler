...
// NewVpaCheckpointsLister returns VerticalPodAutoscalerCheckpointLister configured to fetch all VPA checkpoint objects from namespace,
// set namespace to k8sapiv1.NamespaceAll to select all namespaces.
// The method blocks until vpaCheckpointLister is initially populated.
func NewVpaCheckpointsLister(vpaClient *vpa_clientset.Clientset, stopChannel <-chan struct{}, namespace string) vpa_lister.VerticalPodAutoscalerCheckpointLister {
	vpaCheckpointListWatch := cache.NewListWatchFromClient(vpaClient.AutoscalingV1().RESTClient(), "verticalpodautoscalercheckpoints", namespace, fields.Everything())
	indexer, controller := cache.NewIndexerInformer(vpaCheckpointListWatch,
