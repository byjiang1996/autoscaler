...
func newUsageSample(timestamp time.Time, usage int64, resource ResourceName) *ContainerUsageSampleWithKey {
	return &ContainerUsageSampleWithKey{
...
func TestRecordOOMIncreasedByBumpUp(t *testing.T) {
	...
	assert.NoError(t, test.container.RecordOOM(testTimestamp, containerID, ResourceAmount(1000*mb)))
}
...
func TestRecordOOMDontRunAway(t *testing.T) {
	...
	assert.NoError(t, test.container.RecordOOM(testTimestamp, containerID, ResourceAmount(999*mb)))
	assert.NoError(t, test.container.RecordOOM(testTimestamp, containerID, ResourceAmount(999*mb)))
	...
	assert.NoError(t, test.container.RecordOOM(testTimestamp, containerID, ResourceAmount(2000*mb)))
}
...
func TestRecordOOMIncreasedByMin(t *testing.T) {
	...
	assert.NoError(t, test.container.RecordOOM(testTimestamp, containerID, ResourceAmount(1*mb)))
}
...
func TestRecordOOMDiscardsOldSample(t *testing.T) {
	...
	assert.Error(t, test.container.RecordOOM(testTimestamp.Add(-30*time.Hour), containerID, ResourceAmount(1000*mb)))
}
...
func TestRecordOOMInNewWindow(t *testing.T) {
	...
	test.mockMemoryHistogram.On("AddSample", 2400.0*mb, 1.0, memoryAggregationWindowEnd)
	assert.NoError(t, test.container.RecordOOM(testTimestamp.Add(2*memoryAggregationInterval), containerID, ResourceAmount(1000*mb)))
}
